...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channel, n_class=2, act='relu'):
    ...
    def forward(self, x):
        out64,  skip_out64  = self.down_tr64(x)  # 64 48 48 48 
        out128, skip_out128 = self.down_tr128(out64) # 128 24 24 24
        out256, skip_out256 = self.down_tr256(out128) # 256 * 12 * 12
        out512              = self.down_tr512(out256) # 512 * 12 * 12 * 12 -> 240*240?
...
if __name__ == "__main__":
    ...
    x = torch.ones((2, 3, 96, 96, 48)).cuda()
    model = UNet3D(in_channel=3).cuda()
