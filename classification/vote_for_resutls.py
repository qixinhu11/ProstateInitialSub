...
def test_one_model(model, test_loader):
    ...
    with torch.no_grad():
        for test_data in test_loader:
            ...
            test_preds = torch.sigmoid(test_preds)[0,1]
            test_preds  = test_preds.detach().cpu().numpy()
            # y_pred.append(np.argmax(test_preds))
            y_pred.append(test_preds.item())
...
def voting_models():
    ...
    for modality in ["T2W", "DWI", "ADC", "T2W_ADC", "T2W_DWI", "DWI_ADC", "ALL"]:
        ...
        model.to(device)
        # loading model
        model.load_state_dict(torch.load(f"runs/{modality}/resnet18/model_best.pt", map_location=device))
        # exam the model
        print("-" * 20)
